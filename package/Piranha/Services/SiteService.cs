...
namespace Piranha.Services
{
    public class SiteService : ISiteService
    {
        ...
        /// <summary>
        /// Gets the contacts available for the post with the specified id.
        /// </summary>
        /// <param name="lgId">The unique language id</param>
        /// <param name="onlyApproved">If only approved contacts should be fetched</param>
        /// <param name="page">The optional page number</param>
        /// <param name="pageSize">The optional page size</param>
        /// <returns>The available contacts</returns>
        public Task<IEnumerable<Contact>> GetAllContactsAsync(Guid? lgId = null, bool onlyApproved = true,
            int? page = null, int? pageSize = null)
        {
            return GetAllContactsAsync(lgId, onlyApproved, false, page, pageSize);
        }


        public Task SaveContactAsync(Guid siteId, Contact model)
        {
            return _repo.SaveContact(siteId, model);
        }
        ...
        {
            var model = await GetContactByIdAsync(id).ConfigureAwait(false);

            if (model != null)
            {
                await DeletecontactAsync(model).ConfigureAwait(false);
            }
        }
        ...
        {
            var site = await GetByIdAsync(model.SiteId).ConfigureAwait(false);

            if (site != null)
            {
                // Call hooks & delete
                App.Hooks.OnBeforeDelete<Contact>(model);
                await _repo.DeleteContact(model.Id);
                App.Hooks.OnAfterDelete<Contact>(model);

                // Remove parent post from cache
                RemoveFromCache(site);
            }
            else
            {
                throw new ArgumentException($"Could not find site with id { model.SiteId.ToString() }");
            }
        }
        ...
        /// <param name="onlyApproved">If only approved contacts should be fetched</param>
        /// <param name="onlyPending">If only pending contacts should be fetched</param>
        /// <param name="page">The optional page number</param>
        /// <param name="pageSize">The optional page size</param>
        /// <returns>The available comments</returns>
        private async Task<IEnumerable<Contact>> GetAllContactsAsync(Guid? siteId = null, bool onlyApproved = true,
            bool onlyPending = false, int? page = null, int? pageSize = null)
        {
            ...
            contacts = await _repo.GetAllContact(siteId, onlyApproved, page.Value, pageSize.Value).ConfigureAwait(false);


            // Execute hook
            foreach (var contact in contacts)
            {
                App.Hooks.OnLoad<Contact>(contact);
            }
            return contacts;
        }
